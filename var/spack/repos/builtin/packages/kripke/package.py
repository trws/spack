...
class Kripke(CMakePackage, CudaPackage):
    ...
    tags = ["proxy-app"]
    version("HEAD", submodules=True, branch="develop")
    ...
    variant(
        "chai", default=False, description="Build with CHAI/umpire memory management."
    )
    ...
    def def_prefix_dir(self, name, varname=None):
        return self.define(
            (name if varname is None else varname) + "_DIR", self.spec[name].prefix
        )
    ...
    def cmake_args(self):
        args = [
            ...
            self.dfve("chai"),
            def_prefix_dir('camp'),
            def_prefix_dir('raja', varname='RAJA'),
        ]
        if "+chai" in self.spec:
            args.append(def_prefix_dir('chai'))
            args.append(def_prefix_dir('umpire'))
        return args
